...
export default ({ data,location,pageContext }) => (
  <Layout>
    <SEO
      pagetitle={`${pageContext.displayYear} | 日々のこと`}
      pagedesc={`「${pageContext.displayYear}」アーカイブの記事です`}
          ...
          <div className="column is-mobile-12 is-desktop-9 is-padding-horizontal-lg">
            <div className="blog-list-wrap is-space-xxxl">
              ...
              <ul className="pagenation">
                    ...
                    <Link
                      to={
                        pageContext.currentPage === 2
                          ? `/blog/archive/${pageContext.displayYear}/`
                          : `/blog/archive/${pageContext.displayYear}/${pageContext.currentPage - 1}`
                      }
                      rel="prev"
                    >
                ...
                {!pageContext.isLast && (
                  <li className="next">
                    <Link to={`/blog/archive/${pageContext.displayYear}/${pageContext.currentPage + 1}/`} rel="next">
                      <span>次のページ</span>
...
export const query = graphql`
  query($periodStartDate: Date, $periodEndDate: Date, $skip: Int!, $limit: Int!) {
    allMicrocmsBlog(
      sort: { fields: publishDate, order:DESC }
      skip: $skip
      limit: $limit
      filter: { publishDate: { gte: $periodStartDate, lt: $periodEndDate } }
    ) {
      edges {
        node {
          title
